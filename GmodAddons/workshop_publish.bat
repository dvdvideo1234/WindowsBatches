...
for %%i in %gmadDirs% do (
  echo Extracting: %%i
  call xcopy "!gmadRevPath!%%i" "!gmadRevPath!Workshop\!gmadName!\%%i" /EXCLUDE:!gmadADTools!\workshop\key.txt /E /C /I /F /R /Y
)
...
echo Obtaining the latest repository commit log^^!
call "!gmadPathGIT!\git.exe" rev-parse HEAD>!gmadNameLOG!
call set /p gmadGitHEAD=<!gmadNameLOG!

echo Generating steam workshop commit message^^!
